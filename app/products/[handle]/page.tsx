...
export async function generateStaticParams() {
  ...
  return [
    ...products.map((p: any) => ({ handle: p.handle })),
    ...Object.keys(CATEGORY_MAP).map((slug) => ({ handle: slug })),
  ];
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { handle } = await params;
  if (CATEGORY_MAP[handle]) {
    ...
    <main style={{ background: "#0d1117", minHeight: "100vh" }}>
      ...
      <style>{`
        ...
        @media (min-width: 900px) {
          .product-grid { grid-template-columns: 1fr 1fr !important; align-items: start; }
          .product-right { overflow-y: auto; scrollbar-width: none; }
          .product-right::-webkit-scrollbar { display: none; }
        }
      `}</style>
    </main>
  );
