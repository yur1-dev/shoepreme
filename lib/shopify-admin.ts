...
export async function getProductById(id: string) {
  ...
  if (data.errors) {
    console.error("Shopify product query error:", JSON.stringify(data.errors, null, 2));
    return null;
  }
...
export async function updateVariantPrice(variantId: string, price: string, productId?: string) {
  ...
  if (!resolvedProductId) return { success: false, error: "Could not find product for variant" };
...
export async function createProduct(input: {
  ...
  console.log("createProduct productOptionsCreate response:", JSON.stringify(optionsData, null, 2));
...
export async function addVariantToProduct(
  ...
  if (!hasSizeOption) {
    ...
    const optionErrors = optionsData?.data?.productOptionsCreate?.userErrors;
    console.log("addVariantToProduct productOptionsCreate response:", JSON.stringify(optionsData, null, 2));
    if (optionErrors?.length) {
  ...
  if (variant?.inventoryItem?.id) {
    ...
    if (quantity > 0) {
      await updateVariantInventory(variant.inventoryItem.id, locationId, quantity);
    }
  }
